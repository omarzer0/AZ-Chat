...
) : ViewModel(), AuthRepositoryImpl.AutoVerify {

    private val _event = MutableLiveData<Event<VerificationEvent>>()
    val event: LiveData<Event<VerificationEvent>> = _event

    init {
        repository.registerAutoVerify(this)
    }

    override fun onAutoVerifyDone(uid: String) {
        logMe("onAutoVerifyDone uid:$uid")
        _event.postValue(Event(VerificationEvent.VerificationSuccess(uid)))
    }
